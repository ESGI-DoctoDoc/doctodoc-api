...
public class ManageDoctorAppointmentConfiguration {
    ...
    @Bean
    public ISaveDoctorAppointment saveDoctorAppointment(AppointmentRepository appointmentRepository, UserRepository userRepository, DoctorRepository doctorRepository, GetCurrentUserContext getCurrentUserContext, PatientRepository patientRepository, MedicalConcernRepository medicalConcernRepository, SlotRepository slotRepository, MailSender mailSender) {
        return new SaveDoctorAppointment(doctorRepository, appointmentRepository, medicalConcernRepository, patientRepository, getCurrentUserContext, userRepository, slotRepository, mailSender);
    }
    ...
    @Bean
    public ICancelDoctorAppointment cancelDoctorAppointment(AppointmentRepository appointmentRepository, GetDoctorFromContext getDoctorFromContext, NotificationPushService notificationPushService, PatientRepository patientRepository, MailSender mailSender) {
        return new CancelDoctorAppointment(appointmentRepository, getDoctorFromContext, notificationPushService, patientRepository, mailSender);
    }
